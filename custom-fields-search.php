...
if ( ! function_exists ( 'cstmfldssrch_admin_notices' ) ) {
	function cstmfldssrch_admin_notices() {
		global $hook_suffix, $cstmfldssrch_options;

		$admin_pages = array( 'widgets.php', 'plugins.php', 'update-core.php' );
		if ( in_array( $hook_suffix, $admin_pages ) || ( isset( $_GET['page'] ) && "custom-fields-search.php" == $_GET['page'] ) ) {
			if ( empty( $cstmfldssrch_options ) )
				$cstmfldssrch_options = cstmfldssrch_get_options_default();

			if ( ! $cstmfldssrch_options['display_not_supported_notice'] )
				return;

			if ( isset( $_POST['bws_hide_not_supported_notice'] ) && check_admin_referer( plugin_basename( __FILE__ ), 'bws_settings_nonce_name' ) ) {
				$cstmfldssrch_options['display_not_supported_notice'] = 0;
				update_option( 'cstmfldssrch_options', $cstmfldssrch_options );
				return;
			} ?>
			<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
				<div class="bws_banner_on_plugin_page bws_banner_to_settings">
					<div class="icon">
						<img title="" src="<?php echo plugins_url( 'images/custom-fields-search-icon.png', __FILE__ ); ?>" alt="" />
					</div>
					<div class="text">
						<strong><?php printf( __( '%s becomes %s plugin', 'bestwebsoft' ), 'Custom Fields Search', 'Custom Search' ); ?></strong>
						<br />
						<?php printf( __( '%s plugin is now a part of %s plugin. It will be no longer supported (updates will be unavailable) starting from 23rd April 2018. Install %s plugin now to automatically apply your current settings and get new amazing features.', 'bestwebsoft' ), 'Custom Fields Search', 'Custom Search', 'Custom Search' ); ?>
						<br />
						<a href="https://wordpress.org/plugins/custom-search-plugin/" target="_blank"><?php _e( 'Install Now', 'bestwebsoft' ); ?></a>
					</div>
